...
let storeData = {
    ...
    products: [],
    paymentMethods: ["QRIS", "BCA", "Mandiri", "DANA", "OVO", "ShopeePay"], // Metode Pembayaran Default
    cs: { wa: "", tele: "", ig: "", x: "", tk: "", fb: "", email: "", address: "" },
    soc: { wa: "", tele: "", ig: "", x: "", tk: "", fb: "" },
    
    // TEMPLATE HALAMAN LEGAL (Tetap sama seperti aslinya)
    legal: {
        privacy: `UMUM\n- Kebijakan Privasi ini merupakan kesepakatan antara {{BRAND}} dan Pengguna.`,
        terms: `UMUM\n- Syarat dan Ketentuan ini mengatur penggunaan layanan pada platform {{BRAND}}.`,
        refund: `SIFAT PRODUK DIGITAL\nKarena produk dikirim otomatis, transaksi berhasil tidak dapat dibatalkan.`,
        faq: `1. Apa itu {{BRAND}}?\n{{BRAND}} ({{DOMAIN}}) adalah platform top up digital tepercaya.`
    },
    features: {
        ...
        items: [
            { icon: "fa-solid fa-check-double", title: "Jujur & Tepercaya", desc: "Setiap hari ada ribuan transaksi yang dilakukan oleh pelanggan kami." },
            { icon: "fa-solid fa-bolt", title: "Pengiriman Instan", desc: "Hanya butuh beberapa detik saja untuk menyelesaikan transaksi Anda." },
            ...
            { icon: "fa-solid fa-thumbs-up", title: "Pasti Lebih Murah", desc: "Top up game favorit Anda dengan harga yang pastinya lebih murah." },
...
async function init() {
    try {
        ...
        if (saved) {
            ...
            if (!storeData.features.items || storeData.features.items.length === 0) storeData.features.items = defaultFeatures.items;
            
            // Inisialisasi payment method jika kosong dari DB
            if (!storeData.paymentMethods || storeData.paymentMethods.length === 0) {
                storeData.paymentMethods = ["QRIS", "BCA", "Mandiri", "DANA", "OVO", "ShopeePay"];
            }
...
// ------------------------------------------
// LOGIKA SELEKSI PRODUK & PEMBAYARAN
// ------------------------------------------
...
function processCheckout() {
    ...
    const p = storeData.products.find(item => item.id === selectedProductId);
    if (!p) return;
...
function startCountdown() {
    ...
    countdownInterval = setInterval(() => {
        ...
        document.getElementById('countdown-timer').innerText = `${h < 10 ? "0"+h : h}j ${m < 10 ? "0"+m : m}m ${s < 10 ? "0"+s : s}s`;
    }, 1000);
}
...
function openModal(type) {
    ...
    let parsedText = rawText.replace(/{{BRAND}}/g, brandName).replace(/{{DOMAIN}}/g, domainName).replace(/{{WA}}/g, csWA).replace(/{{EMAIL}}/g, csEmail);
...
// ---------------------------------------------
// FUNGSI ADMIN PAYMENT METHOD
// ---------------------------------------------
function renderAdminPayments() {
    const list = document.getElementById('admin-payment-list');
    if(!list) return;
    list.innerHTML = storeData.paymentMethods.map((m, i) => `
        <div class="payment-admin-item">
            <span style="font-size: 0.85rem; font-weight: 600; color: var(--text);">${m}</span>
            <button onclick="removePaymentMethod(${i})" style="background: transparent; border: none; color: var(--danger); cursor: pointer;"><i class="fa-solid fa-circle-xmark"></i></button>
        </div>
    `).join('');
}
...
async function addPaymentMethod() {
    ...
    if(val) {
        storeData.paymentMethods.push(val);
        input.value = '';
        await saveToDB(storeData);
        renderAdminPayments();
        renderCustomer(); // Refresh agar customer langsung melihatnya
    }
}

async function removePaymentMethod(index) {
    if(confirm("Hapus metode pembayaran ini?")) {
        storeData.paymentMethods.splice(index, 1);
        await saveToDB(storeData);
        renderAdminPayments();
        renderCustomer();
    }
...
function populateAdmin() {
    ...
    populateDatalists();
    renderAdminList();
    renderAdminPayments(); // Load data metode pembayaran di admin
}
...
async function saveGeneralSettings() {
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        if (input && input.files && input.files[0]) {
            storeData.banners[i - 1] = await toBase64(input.files[0]);
        }
    }
    ...
    for (let i = 1; i <= 8; i++) {
        ...
        if (input && input.files && input.files[0]) {
            storeData.customerCarousel[i - 1] = await toBase64(input.files[0]);
        }
    }
